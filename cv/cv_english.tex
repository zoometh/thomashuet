...
\begin{center}(\textbf{i}) international = 9 {\textbar} (\textbf{n}) national audience = 8 \end{center}
\smallbreak
...
\smallbreak
\textbf{2021 }"Paisatges de conflicte i de poder. Els gravats medievals del Sol\`{a} de Saur\`{i} (Pallars Sobir\`{a})", co-presented with E. Gassiot Ballb\`{e} and O. Aug\'{e} Mart\'{i}nez, Tribuna d'Arqueologia, Barcelona, 21 April. \textbf{(n)}
\smallbreak
...
\textsf{R}, \textsf{Python}, \textsf{JavaScript}, SQL, markup languages (JSON, XML, HTML, CSS, etc.)~\textbf{{\textbar}} PostgreSQL/GIS, NoSQL databases, FileMaker, SpatiaLite, Access
