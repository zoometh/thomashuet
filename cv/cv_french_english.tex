...
% non affiché
\fr{Thomas Huet est ingénieur de recherche au CNRS. Il a travaillé dans des domaines très variés, en plaçant au centre de son activité la gestion de l'information archéologique, notamment à travers l'infrastructuration (administration système, architecture client-serveur, bases de données), l'interopérabilité (API, thésaurus, ontologies, métadonnées) et la modélisation (analyse de réseaux, statistiques).
}
\en{Thomas Huet is a Research Engineer at the \textit{Centre National de la Recherche Scientifique} (CNRS). He has worked in a wide array of fields, focusing his work on the management of archaeological information, particularly through IT infrastructure (system administration, client-server architecture, databases), interoperability (APIs, thesauri, ontologies, metadata), and modeling (network analysis, statistics).}
