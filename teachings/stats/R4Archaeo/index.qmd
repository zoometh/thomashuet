...
R Markdown extension is `.Rmd`: `R` + `md` for Markdown
...
<tbody>
  <tr>
    <td>![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/r-pkg-knitr.png){height=70px}</td>
    <td>[knitr](https://yihui.org/knitr/) engine for dynamic report generation with R (e.g. convert to LaTeX)</td>
  </tr>
  <tr>
    <td>![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/r-pkg-rmarkdown.png){height=70px}</td>
    ...
    <tr>
    <td>![](https://raw.githubusercontent.com/zoometh/thomashuet/master/img/r-pkg-bookdown.png){height=70px}</td>
...
<tbody>
  ...
  <tr>
    ...
    <td>run [Visual R Markdown](#vrm) + other options</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>run [Visual R Markdown](#vrm)</td>
  </tr>
</tbody>
</table>
...
Cite and reuse variables, figures, tables, sections, etc., through your document with dynamic locators
...
```{r maxLmaxB, echo=TRUE, fig.align="center", fig.cap="Maximum Length/Maximum breadth in cm"}
...
BibTex, or `.bib`, is a plain text format for bibliographic references. Google Scholar is a nice tool to copy/paste BibTex references, e.g.:

```{.md}
published by Yihui Xie [-@Xie19]
```

It's easy to retrieve [this reference](https://scholar.google.fr/scholar?hl=en&as_sdt=0%2C5&q=Tinytex%3A+A+Lightweight%2C+Cross-Platform%2C+and+Easy-to-Maintain+Latex+Distribution+Based+on+Tex+Live&btnG=)
...
<img src="www/rmd_create_rmd_new-bib.png" alt="rmd_yaml_bib" width="600"/>
...
R code chunks are are placeholders for the coding part of the document, expressed with a local header, and a body part.
...
The first chunk in the document allows to set these options for all other chunks unless you change these options in the following code chunks headers
...
<tbody>
  ...
  <tr>
    <td>![](www/rmd_codechunk_header_runabove.png){width="60px"}</td>
    <td>run the previous chunks but not this one</td>
  </tr>
  <tr>
    <td>![](www/rmd_codechunk_header_runthis.png){width="60px"}</td>
...
Use R Markdown (code chunks and narrative text) to create a report, choose between:

- [ ] improve the existing `.Rmd` template for the La Tène fibulae from the Iron Age cemetery of Münsingen near Berne, Switzerland (100 - 500 BCE)

![](www/munsingen_fib_measures.png)

- [ ] choose another dataset from the {`archdata`} package (e.g. 'Handaxes'), to improve the `.Rmd` template
...
Go there <https://github.com/zoometh/thomashuet/tree/main/teachings/stats/R4Archaeo/practice> and download, or copy/paste the `.Rmd` file and the .`bib` file.
